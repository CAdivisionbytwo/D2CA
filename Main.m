...
fId = fopen('Results1.csv', 'w');
fprintf(fId, ' ,Alg1_Vec,Alg1_For,Alg3_For,Alg3_ParFor@2,Alg3_ParFor@3,Alg3_ParFor@4');
...
for nd = 1:length(nDigits)
    ...
    for iter = 1:MaxIter
        ...
        [~, exeTime1_Vec] = Alg1_Vector(longNumber_Array); %parallel with vector
        [~, exeTime1_For] = Alg1_For(longNumber_Array);  %serial
        [~, exeTime3_For] = Alg3_For(longNumber_Array); %serial 
        [~, exeTime3_ParFor2] = Alg3_ParFor(longNumber_Array, 2); % parallel with par
        [~, exeTime3_ParFor3] = Alg3_ParFor(longNumber_Array, 3);  % parallel with par
        [~, exeTime3_ParFor4] = Alg3_ParFor(longNumber_Array, 4);  % parallel with par
